...
from .booking_utils import is_time_slot_available
...
def make_bookings(request, salon_service_id):
    ...
    This view handles information about the make bookings page
    ...
    salon_service = SalonService.objects.get(id=salon_service_id)
    print('Helooo?')
    # messages.error(request, 'This is a test error message.')  # This should always show
    ...
    if request.method == 'POST':
        ...
        if not is_time_slot_available(salon_service, date, start_time, end_time):
            # If the time slot is not available, display an error message
            print('Time slot unavailable')  # Debugging
            messages.error(request, 'The selected time slot is not available. Please choose another time.')
            return render(request, 'bookings/make_bookings.html', {'salon_service': salon_service})

        # Create a new booking if the time slot is available
        booking = Booking.objects.create(
            customer=customer,
            salon_service=salon_service,
            date=date,
        ...
        print(f"Booking created for {customer.user.username} on {date} at {start_time}")
